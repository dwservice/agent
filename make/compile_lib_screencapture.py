...
CONF_WINDOWS["cpp_include_paths"]=[utils.PATHTMP + os.sep + "lib_z"]
CONF_WINDOWS["cpp_library_paths"]=CONF_WINDOWS["cpp_include_paths"]
CONF_WINDOWS["libraries"]=["zlib1", "gdi32", "userenv"]
CONF["windows"]=CONF_WINDOWS

CONF_LINUX={}
CONF_LINUX["outname"]="dwagscreencapture.so" 
CONF_LINUX["cpp_include_paths"]=[utils.PATHTMP + os.sep + "lib_z"] 
CONF_LINUX["cpp_library_paths"]=CONF_LINUX["cpp_include_paths"]
CONF_LINUX["libraries"]=["X11", "z", "Xext", "dl", "Xtst"]
CONF["linux"]=CONF_LINUX

CONF_MAC={}
CONF_MAC["outname"]="dwagscreencapture.so" 
CONF_MAC["cpp_include_paths"]=[utils.PATHTMP + os.sep + "lib_z"] 
CONF_MAC["cpp_library_paths"]=CONF_MAC["cpp_include_paths"]
CONF_MAC["libraries"]=["z"]
...
class Compile():
    ...
    def run(self):
        ...
        if confos is not None:
            if utils.is_mac(): 
                utils.system_exec(["install_name_tool -change \"/usr/local/lib/libz.1.so\" \"@loader_path/libz.so\" " + confos["outname"]], CONF["pathdst"])        
